...
export class BlogService {
  ...
  async createDraft(dto: CreateBlogPostDraftDto): Promise<BlogPost> {
    const title = dto.title?.trim() || 'New Blog Post';
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are an assistant that edits blog post content. Return only the final HTML body fragment.',
          },
          {
            role: 'user',
            content: `Instruction:
...
${content || ''}`,
  ...
  async findOne(id: number): Promise<any | null> {
    ...
    if (post)
      await this.redisClient.set(cacheKey, JSON.stringify(post), 'EX', 3600);
    return post;
  }
