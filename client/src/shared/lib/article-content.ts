...
export type ArticleBlock = OutputBlockData & {
...
function blockFromMedia(data: Record<string, unknown>, id: string): ArticleBlock {
  return { id, type: "media", data };
}

function blockFromSlider(data: Record<string, unknown>, id: string): ArticleBlock {
  return {
    id,
    type: "slider",
    data: {
...
function blockFromWidget(data: Record<string, unknown>, id: string): ArticleBlock {
  return {
    id,
    type: "widget",
    data: {
...
function tryParseMarkerLine(line: string): ArticleBlock | null {
...
export function blocksFromArticle(article: ArticleDto): ArticleBlock[] {
  if (article.content) {
    try {
    ...
    if (
      article.content.includes("[[media:") ||
      article.content.includes("[[slider:") ||
      article.content.includes("[[widget:") ||
      article.content.includes("📷") ||
      article.content.includes("🎠") ||
      article.content.includes("🧩")
    ) {
      return markdownToBlocks(article.content);
    }
  }

  if (article.contentBlocks?.length) {
    return article.contentBlocks.map((block, index) => ({
      id: String(block.id ?? index),
      type: block.type,
