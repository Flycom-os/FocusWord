...
export type BlogPostBlock = OutputBlockData & {
...
function blockFromMedia(data: Record<string, unknown>, id: string): BlogPostBlock {
  return { id, type: "media", data };
}

function blockFromSlider(data: Record<string, unknown>, id: string): BlogPostBlock {
  return {
    id,
    type: "slider",
    data: {
...
function blockFromWidget(data: Record<string, unknown>, id: string): BlogPostBlock {
  return {
    id,
    type: "widget",
    data: {
...
export function blocksToMarkdown(blocks: BlogPostBlock[]): string {
...
function tryParseMarkerLine(line: string): BlogPostBlock | null {
...
export function markdownToBlocks(markdown: string, keepBlocks?: BlogPostBlock[]): BlogPostBlock[] {
  if (!markdown?.trim()) {
    return keepBlocks?.filter((b) => b.type === "media" || b.type === "slider" || b.type === "widget") || [];
  }

  const result: BlogPostBlock[] = [];
...
export function blocksFromBlogPost(blogPost: BlogPostDto): BlogPostBlock[] {
  if (blogPost.content) {
    try {
    ...
    if (
      blogPost.content.includes("[[media:") ||
      blogPost.content.includes("[[slider:") ||
      blogPost.content.includes("[[widget:") ||
      blogPost.content.includes("📷") ||
      blogPost.content.includes("🎠") ||
      blogPost.content.includes("🧩")
    ) {
      return markdownToBlocks(blogPost.content);
    }
  }

  if (blogPost.contentBlocks?.length) {
    return blogPost.contentBlocks.map((block, index) => ({
      id: String(block.id ?? index),
      type: block.type,
...
export function serializeBlogPostBlocks(blocks: BlogPostBlock[]): string {
