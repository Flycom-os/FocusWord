...
import styles from "./blog-admin-view.module.css";

const Editor = dynamic(() => import("@/src/features/Editor/ui/Editor"), { ssr: false });

const defaultQuery: BlogQuery = { page: 1, limit: 20 };
...
const BlogBlogPost = () => {
  ...
  const [query, setQuery] = useState<BlogQuery>(defaultQuery);
  const [search, setSearch] = useState("");
  const [blog, setBlog] = useState<BlogPostDto[]>([]);
  const [isLoading, setIsLoading] = useState(false);

  // Table selection hook
  const tableSelection = useTableSelection({
    items: blog,
    getItemId: (blogPost) => blogPost.id,
    onSelectionChange: (selectedIds) => {
      console.log("Selected blogd:", selectedIds);
    },
  });

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingBlogPost, setEditingBlogPost] = useState<BlogPostDto | null>(null);
  ...
  const totalBlog = useMemo(() => {
    ...
    return Math.max(1, Math.ceil((blog.length || 1) / query.limit));
  }, [blog.length, query.limit]);

  const loadBlog = async () => {
    if (!accessToken) return;
    setIsLoading(true);
    try {
      const data = await fetchBlog(accessToken, {
        ...query,
        search: debouncedSearch || undefined,
      });
      setBlog(data);
    } catch (error: any) {
      showToast(error?.response?.data?.message || "Не удалось загрузить блога", "error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleEditBlogPost = (blogPost: BlogPostDto) => {
    window.location.href = `/admin/blog/create/${blogPost.id}`;
  };
  ...
  const handleAiAssist = async () => {
    ...
    try {
      ...
      const content = editorData.blocks
        ...
        .filter(Boolean)
        .join("\n");
      const result = await completeBlogPostWithAi(accessToken, { prompt: prompt.trim(), content });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingBlogPost) {
        const updated = await updateBlogPost(accessToken, editingBlogPost.id, blogPostData);
        setBlog((prev) => prev.map((item) => (item.id === updated.id ? updated : item)));
        showToast("Запись блога обновлена", "success");
      } else {
        const created = await createBlogPost(accessToken, blogPostData);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteBlogPost(accessToken, id);
      setBlog((prev) => prev.filter((item) => item.id !== id));
      showToast("Запись блога удалена", "success");
    } catch (error: any) {
      showToast(error?.response?.data?.message || "Не удалось удалить запись блога", "error");
    }
  };
    ...
    <div className={styles.root}>
      ...
      <div className={styles.toolbar}>
        <Input
          ...
          theme="secondary"
          icon="left"
          placeholder="Поиск записей блога..."
          value={search}
          onChange={(event) => setSearch(event.target.value)}
        />
        <PermissionGate resource="blog" level={2}>
          <UiButton theme="primary" onClick={handleCreateBlogPost}>
      ...
      <Table className={styles.table} ref={tableSelection.tableRef}>
        ...
        <TableBody>
          {isLoading ? (
            ...
            <TableRow>
              <TableCell colSpan={5}>Блог не найдены</TableCell>
            </TableRow>
          ) : (
            blog.map((blogPost, index) => (
              <SelectableTableRow
                key={blogPost.id}
                selected={tableSelection.isSelected(blogPost.id)}
                focused={tableSelection.focusedIndex === index}
                onSelect={(e) => tableSelection.handleRowClick(blogPost.id, e)}
                checkboxColumn
              >
                <TableCell>{blogPost.title}</TableCell>
                <TableCell>{blogPost.slug}</TableCell>
                <TableCell>
                  {blogPost.status === "published" ? "Опубликовано" : "Черновик"}
                </TableCell>
      ...
      <PermissionGate resource="blog" level={2}>
        <Modal
          ...
          title={editingBlogPost ? "Редактировать запись блога" : "Создать запись блога"}
        >
      ...
      <Modal
        ...
        title="Предпросмотр блога"
      >
        <div className={styles.previewModalContent}>
          <div className={styles.previewHeader}>
            <h2>{form.title || "Предпросмотр блога"}</h2>
