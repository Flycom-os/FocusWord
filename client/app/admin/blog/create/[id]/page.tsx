...
import { updateBlogPost, fetchBlogPost } from "@/src/shared/api/blog";
import { fetchSliders, getSlider, SliderDto, SliderDetailsDto } from "@/src/shared/api/sliders";
import { BlogPostPreviewModal } from "@/src/widgets/blogPost-preview/BlogPostPreviewModal";
...
import {
  ...
  blocksToMarkdown,
  markdownToBlocks,
  serializeBlogPostBlocks,
  type BlogPostBlock,
} from "@/src/shared/lib/blogPost-content";
...
const EditBlogPostPage = ({ params }: { params: { id: string } }) => {
  ...
  const [blogPostId] = useState(params.id);
  ...
  useEffect(() => {
    const loadBlogPostData = async () => {
      if (!accessToken || !blogPostId) return;
      setIsLoading(true);
      try {
        const blogPost = await fetchBlogPost(accessToken, parseInt(blogPostId, 10));
        setForm({
          title: blogPost.title || "",
          slug: blogPost.slug || "",
          status: blogPost.status || "draft",
          template: blogPost.template || "default",
          seoTitle: blogPost.seoTitle || "",
          seoDescription: blogPost.seoDescription || "",
          metaKeywords: (blogPost.metaKeywords || []).join(", "),
          featuredSliderId: blogPost.featuredSliderId || null,
          categoryIds: (blogPost.categories || []).map((c: any) => c.id),
          enableFeedback: blogPost.enableFeedback !== false,
          paymentMethodId: blogPost.paymentMethodId || null,
        });

        const blocks = blocksFromBlogPost(blogPost);
        setEditorData({ blocks });

        if (blogPost.featuredSliderId) {
          try {
            setSelectedSlider(await getSlider(accessToken, blogPost.featuredSliderId));
          } catch {
            const slider = sliders.find((s) => s.id === blogPost.featuredSliderId);
            setSelectedSlider(slider ? { ...slider, slides: [] } : null);
          }
        }
      } catch {
        showToast("Не удалось загрузить запись блога", "error");
    ...
    if (accessToken) {
      loadSliders()
        ...
        .then(() => loadBlogPostData());
    }
  }, [accessToken, blogPostId]);
  ...
  const handleMediaSelect = () => {
    window.dispatchEvent(
      new CustomEvent("open-media-picker", {
        detail: {
          onSelect: (media: any) => {
            const mediaBlock: BlogPostBlock = {
            ...
            const currentBlocks = (editorData?.blocks as BlogPostBlock[]) || [];
  ...
  const handleSliderSelect = () => {
    ...
    (window as any).selectSliderEdit = (slider: SliderDto) => {
      const sliderBlock: BlogPostBlock = {
        id: Date.now().toString(),
        type: "slider",
        data: {
          sliderId: slider.id,
  ...
  const handleWidgetSelect = () => {
    ...
    (window as any).selectWidgetEdit = (w: WidgetDto) => {
      const widgetBlock: BlogPostBlock = {
        id: Date.now().toString(),
        type: "widget",
        data: {
          slug: w.slug,
  ...
  const handleSave = async () => {
    ...
    try {
      const blocks = (editorData?.blocks as BlogPostBlock[]) || [];
      await updateBlogPost(accessToken, parseInt(blogPostId, 10), {
        ...
        content: blocks.length ? serializeBlogPostBlocks(blocks) : "",
      });
      showToast("Запись блога сохранена", "success");
      router.push("/admin/blog");
    } catch (error: any) {
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <div className={styles.breadcrumb}>
          ...
          <h1>Редактировать запись блога</h1>
        </div>
