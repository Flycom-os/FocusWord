...
import { createRecord } from '@/src/shared/api/records';
...
export default function CreateRecordPage() {
  ...
  const [form, setForm] = useState({
    ...
    seoDescription: '',
    metaKeywords: '',
    featuredSliderId: undefined as number | undefined,
  });
  ...
  const handleSliderSelect = () => {
      ...
      <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 2000;">
        <div style="background: white; border-radius: 12px; max-width: 500px; width: 90%; max-height: 80vh; overflow-y: auto; padding: 24px;">
          ...
          <div style="display: flex; flex-direction: column; gap: 12px;">
            ${sliders.map(slider => `
              <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; cursor: pointer; transition: all 0.2s;" 
                   ...
                   onclick="selectSliderRecordCreate(${JSON.stringify(slider).replace(/"/g, '&quot;')})">
          ...
          <div style="margin-top: 20px; display: flex; justify-content: flex-end; gap: 12px;">
            <button onclick="closeSliderModalRecordCreate()" style="padding: 8px 16px; border: 1px solid #d1d5db; background: white; border-radius: 6px; cursor: pointer;">Отмена</button>
          </div>
        </div>
      </div>
    `;
    ...
    (window as any).selectSliderRecordCreate = (slider: any) => {
      ...
      setEditorData(newData);
      (window as any).closeSliderModalRecordCreate();
    };
    
    (window as any).closeSliderModalRecordCreate = () => {
      document.body.removeChild(modalDiv);
      delete (window as any).selectSliderRecordCreate;
      delete (window as any).closeSliderModalRecordCreate;
    };
  };
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <p>Создайте новую запись для сайта</p>
      </div>
