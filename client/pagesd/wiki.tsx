...
const WikiPage = () => {
  ...
  useEffect(() => {
    const loadPage = async () => {
      console.log('WikiPage: Loading page with slug: wiki');
      
      try {
        const pageData = await fetchPublicPageBySlug('wiki');
        setPage(pageData);
      } catch (error: any) {
        console.error('WikiPage: Failed to load page', error);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="app light">
        ...
        <Body>
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-8">Wiki страница не найдена</p>
            <Link 
              href="/"
